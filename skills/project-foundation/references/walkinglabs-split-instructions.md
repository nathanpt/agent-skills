...
```mermaid
flowchart LR
Mono["One giant AGENTS.md"] --> MonoLoad["Even a small bug fix  
means reading all deploy rules and old notes"]
MonoLoad --> MonoRisk["Critical rules buried in the middle  
easily missed"]
Router["Short AGENTS.md"] --> Topics["Read API / DB / testing docs  
only when this task needs them"]
Topics --> RoutedResult["More context left for code reading  
and verification"]
```
```mermaid
flowchart TB
File["600-line instruction file"] --> Top["Top section  
quick start + hard constraints"]
File --> Mid["Middle section  
security rule at line 300"]
File --> Bot["Bottom section  
explicit end-of-file checklist"]
